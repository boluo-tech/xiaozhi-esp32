...
private:
    ...
    TaskHandle_t lyric_task_handle_;  // 歌词显示任务句柄
    ...
    size_t buffer_size_;
    static constexpr size_t MAX_BUFFER_SIZE = 256 * 1024;  // 256KB缓冲区（优化内存）
    static constexpr size_t MIN_BUFFER_SIZE = 32 * 1024;   // 32KB最小播放缓冲（优化内存）
    ...
    int current_playlist_index_ = 0;
    TaskHandle_t playlist_task_handle_;  // 歌单播放任务句柄
