...
#define TAG "JPetAudioCodec"


JPetAudioCodec::JPetAudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    es8388_cfg.pa_pin = pa_pin;// 外部控制切换MIC
    ...
    ESP_LOGI(TAG, "JPet AudioDevice initialized");
}

JPetAudioCodec::~JPetAudioCodec() {
...
void JPetAudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
...
void JPetAudioCodec::SetOutputVolume(int volume) {
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, volume));
    AudioCodec::SetOutputVolume(volume);
}

void JPetAudioCodec::EnableInput(bool enable) {
...
void JPetAudioCodec::EnableOutput(bool enable) {
...
int JPetAudioCodec::Read(int16_t* dest, int samples) {
    if (input_enabled_) {
...
int JPetAudioCodec::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
